...
These HTTP(S) interception and mocking techniques are super useful for testing and understanding most apps, but they have issues with the small set of hyper-vigilant apps that add extra protections aiming to defeat that.
...
Protections like certificate pinning make this difficult. Let's talk about how you can fight back, by using [Frida](https://frida.re/) to remove SSL pinning and expose the real traffic that any app is sending.

## What's certificate pinning?

By default, when an Android app makes an HTTPS connection, it makes sure that it's talking to a trusted server by comparing the issuer of the server's certificate to Android's built-in list of system certificates.

99% of apps stick with that default. You can't change the system certificates list on normal devices, so this list of certificates is fairly reliable and secure. You can change it though on rooted devices and most emulators, so it's quite possible to intercept and inspect HTTPS traffic from these apps by using a debugging proxy for HTTPS interception in those environments.

Unfortunately however, the last 1% which don't stick with the default configuration are often some of the most interesting. These apps include their own custom certificate validation, to more specify the _exact_ HTTPS certificate issuers they're prepared to trust, instead of trusting all standard certificate authorities. This ensures they will never trust a new certificate from a certificate authority that they don't explicitly recognize, and so won't accidentally expose HTTPS traffic to anybody other than the real server.
...
Certificate pinning used to be a much more popular technique, back before Android Nougat when Android's own certificate validation was more lax and users could easily by tricked into installing new trusted certificates on their devices. Nowadays certificate pinning is much rarer, since (as we'll see) it's really [security theater](https://en.wikipedia.org/wiki/Security_theater), and Google's own docs specifically [recommend against](https://developer.android.com/training/articles/security-ssl#Pinning) the practice:
...
[Frida](https://frida.re/) is a cross-platform multi-purpose framework for dynamically transforming how applications work from the outside. Think [Greasemonkey](https://addons.mozilla.org/en-US/firefox/addon/greasemonkey/), but for programs instead of web pages.
...
Frida supports Android, primarily using on-device server that runs on rooted devices, and exposes an API via ADB so you can use Frida's CLI tools on your computer to transform apps on your phone on the fly.
...
Your emulator can use any Android version (although a recent version matching your machine's architecture is a good idea), but **must use a non-'Google Play' build**. Either Vanilla or 'Google APIs' is fine, but Google Play builds include restrictions similar to physical devices that limit debugging.
...
Writing these scripts is quite complicated. There's many examples of small individual scripts available designed to remove pinning from specific target apps or certain HTTPS libraries, but not many that try to remove pinning for )_all_ HTTPS traffic.
...
This will restart the app on your phone, and print out a series of libraries where unpinning was attempted, with a `[+]` for libraries that were successfully patched and `[ ]` for libraries that weren't (generally because the app doesn't use those libraries). When any unpinned methods are called, you'll also see an extra `--> Bypassing [pinning method]` message, to let you know which APIs this app is using.
...
That means that an HTTP client (the Twitter app) is connecting and then rejecting the certificate immediately, without sending any requests. Each of these apps provides some errors like this, for the specific hosts who've certificates have been pinned within the app.
...
Jackpot! From here, you can explore the content of each of those requests, or add rules to rewrite, mock or block them entirely.
