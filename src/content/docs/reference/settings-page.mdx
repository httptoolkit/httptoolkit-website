...
There are five cards here, each of which can be collapsed or expanded as required by clicking or pressing enter on the title and chevron in the top right. The cards are:
...
* The API Settings card, which allows you to add your own OpenAPI specifications.
...
Some HTTPS connections can't be intercepted, usually because the client refuses to trust the HTTP Toolkit certificate. Here you can list hostnames whose TLS connections should be passed straight upstream, untouched. That can resolve the resulting connection failures, at the cost of not being able to see inside those connections at all: they'll appear on the View page as TLS tunnels instead of exchanges.

Hostnames can include `*` wildcards, so you can exclude an entire domain at once.

You can also invert this, using the toggle beside the heading. In 'Intercept only these hostnames' mode, the list becomes the only hostnames that _will_ be intercepted, and every other TLS connection is passed through. That's useful when you're interested in one specific service and want to leave everything else on the machine untouched.

Either way, non-TLS traffic is always visible regardless of this setting.

### TLS Key Log File

If you set a file path here, the TLS keys for all client & server traffic will be logged to that file.

This allows low-level packet inspection tools like Wireshark to decrypt the raw TLS packets they capture, so you can examine traffic below the HTTP layer alongside HTTP Toolkit's own view of it.
...
* HTTP - a custom HTTP proxy will be used
* HTTPS - a custom HTTPS proxy will be used
* SOCKS - a custom SOCKS proxy will be used, selecting between SOCKS 4, 4a, 5 or 5h

When a custom proxy is selected, additional configuration options are shown, to allow you to enter the proxy address, and to add a list of non-proxied hosts whose traffic should always be sent directly, ignoring the proxy.
...
If your servers use certificates issued by a private certificate authority, such as an internal corporate CA, HTTP Toolkit won't trust them by default and those connections will fail.

Here you can load extra CA certificates (in `.pem`, `.crt` or `.cert` format) which will be treated as trusted roots for all HTTPS requests, in addition to the normal set of built-in trusted CAs.

This is different to the host whitelist below: that disables validation entirely for the hosts listed, whilst this keeps validation on but trusts an additional CA, making that working HTTPS servers using certs from that CA are accessible while preserving all the other security of TLS. This is generally the better and more secure option.
...
HTTP Toolkit includes built-in specifications for 2600+ popular public APIs, which power the API card on [the View page](/docs/reference/view-page/).

This card lets you add your own. Load an OpenAPI or Swagger specification, enter the base URL that requests to this API will use, and save it. From then on, any request within that base URL will get the same documentation & validation as the built-in APIs.
