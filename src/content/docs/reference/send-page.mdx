...
When you send a request, HTTP Toolkit sends it directly to the server (not via HTTP Toolkit's proxy) so it's not affected by your modify rules, and you'll see the real behaviour of the request.
...
Similarly to modify rules, requests defined on the Send page are only stored persistently with [HTTP Toolkit Pro](/get-pro/). For hobbyist users, this pane resets every time the app restarts.
