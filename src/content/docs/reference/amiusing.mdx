...
This is powered by a rule, shown on [the Modify page](/docs/reference/modify-page/) in the HTTP Toolkit app, and can be changed or removed. The built-in page content may also change in future, but if you want to reliably detect this programmatically there's a few options:
...
Typically this means that your HTTP client is not using the proxy settings for this connection. It's also possible that you've removed, edited or overridden the rule that powers this page.
